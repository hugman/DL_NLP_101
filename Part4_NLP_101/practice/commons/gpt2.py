...
from commons.base import clones, cp_weight
...
class GPT2(nn.Module):
    ...
    def copy_weights_from_huggingface(self, hg_model):
        ## src: huggingface GPT2 - Transformer model 
        ## tar: my GPT2 - model - core weights

        ## [INPUT EMBEDDING] 
        ## copy embeddings from huggingface to my gpt2
        self.wte.load_state_dict( hg_model.transformer.wte.state_dict() )
        self.wpe.load_state_dict( hg_model.transformer.wpe.state_dict() )

        ## [OUTPUT EMBEDDING]
        ## copy to output vocab
        self.head.load_state_dict( hg_model.lm_head.state_dict() )

        ## [TRANSFORMER BLOCK]
        ## layer normalization at top transformer block 
        cp_weight(self.ln_f, hg_model.transformer.ln_f, include_eps=True) # ln_f

        ## layer weights
        for layer_num, src_block in enumerate(hg_model.transformer.h):
            # <<< MultiHeadAttention (Conv1D's parameters) >>>
            cp_weight(src_block.attn.c_attn, self.blocks.layers[layer_num].attn.c_attn) # c_attn
            cp_weight(src_block.attn.c_proj, self.blocks.layers[layer_num].attn.c_proj) # c_proj

            # <<< MLP >>
            cp_weight(src_block.mlp.c_fc,       self.blocks.layers[layer_num].mlp.c_fc) # c_fc
            cp_weight(src_block.mlp.c_proj,     self.blocks.layers[layer_num].mlp.c_proj) # c_proj
            #tar.blocks.layers[layer_num].mlp.dropout.load_state_dict(src_block.mlp.dropout) # dropout

            # layer normalization parameters
            cp_weight(src_block.ln_1, self.blocks.layers[layer_num].ln_1, include_eps=True) # ln_1
            cp_weight(src_block.ln_2, self.blocks.layers[layer_num].ln_2, include_eps=True) # ln_2
...
def cli_main():
    ...
    my_model = my_model.cp_decoder_block_weights_from_huggingface(hg_model)
